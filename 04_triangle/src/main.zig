...
pub fn main() !void {
    ...
    var sized = try createSizedResources(&manager, allocator, command_pool, swapchain, vertex_buffer.buffer);
    ...
    while (!window.shouldClose()) {
        ...
        if (present_state == .suboptimal or desired_extent.width != extent.width or desired_extent.height != extent.height) {
            ...
            sized = try createSizedResources(&manager, allocator, command_pool, swapchain, vertex_buffer.buffer);
            extent = swapchain.extent;
            present_state = .optimal;
        }

        const angle: f32 = @floatCast(glfw.getTime() - start_time);

        try recordFrame(
            &manager,
            sized.command_buffers[swapchain.image_index],
            swapchain.extent,
            sized.render_pass,
            sized.pipeline_layout,
            sized.pipeline,
            sized.framebuffers[swapchain.image_index],
...
fn createSizedResources(
    ...
    errdefer manager.device.?.destroyPipelineLayout(pipeline_layout, null);
    ...
    const command_buffers = try allocateCommandBuffers(manager, allocator, command_pool, framebuffers.len);
    errdefer {
