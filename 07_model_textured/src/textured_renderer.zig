...
const model_glb_bytes align(4) = @embedFile("model_glb").*;
...
const ModelData = struct {
    ...
    fn deinit(self: *ModelData) void {
        ...
        self.allocator.free(self.texture_bytes);
    }
};
...
pub const TexturedRenderer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) TexturedRenderer {
        return .{ .allocator = allocator };
    }

    pub fn createOnce(self: *TexturedRenderer, session: *ash.Session) !void {
        const manager = &session.manager.?;
        const device = manager.device orelse return error.DeviceNotInitialized;
        self.device = device;

        self.model = try loadTeacupModel(self.allocator);
        std.log.info("loaded model: {} vertices, {} indices, texture bytes {}", .{
            self.model.?.vertices.len / floats_per_vertex,
            self.model.?.indices.len,
            self.model.?.texture_bytes.len,
        });
    ...
    pub fn destroyOnce(self: *TexturedRenderer) void {
        if (!self.once_built) {
            return;
        }
    ...
    pub fn createSized(self: *TexturedRenderer, session: *ash.Session, extent: vk.Extent2D) !void {
        const manager = &session.manager.?;
        const device = manager.device orelse return error.DeviceNotInitialized;
    ...
    pub fn destroySized(self: *TexturedRenderer) void {
        if (!self.sized_built) {
            return;
        }
        const device = self.device orelse return;
    ...
    pub fn draw(self: *TexturedRenderer, session: *ash.Session, frame: *const ash.Frame) !void {
        const device = session.manager.?.device orelse return error.DeviceNotInitialized;

        const elapsed = @as(f32, @floatCast(ash.glfw.getTime() - self.start_time)) * 30.0;
        ...
        const clear_values = [_]vk.ClearValue{
            .{ .color = .{ .float_32 = .{ 0.15, 0.15, 0.18, 1.0 } } },
...
fn loadTeacupModel(allocator: std.mem.Allocator) !ModelData {
    var gltf = zgltf.Gltf.init(allocator);
    defer gltf.deinit();

    try gltf.parse(&model_glb_bytes);

    const binary = gltf.glb_binary orelse return error.GltfMissingBinary;
    ...
    for (gltf.data.meshes) |mesh| {
        for (mesh.primitives) |primitive| {
            ...
            const uv_idx = texcoord_index orelse return error.GltfMissingTexCoords;

            const pos_accessor = gltf.data.accessors[pos_idx];
            const norm_accessor = gltf.data.accessors[norm_idx];
            const uv_accessor = gltf.data.accessors[uv_idx];
            if (pos_accessor.count != norm_accessor.count or pos_accessor.count != uv_accessor.count) {
            ...
            var uv_it = uv_accessor.iterator(f32, &gltf, binary);
            var i: usize = 0;
            while (i < vertex_count) : (i += 1) {
                ...
                const uv = uv_it.next() orelse return error.GltfTexCoordShort;
    ...
    const texture_bytes = try extractBaseColorTextureBytes(allocator, &gltf);
    errdefer allocator.free(texture_bytes);
...
fn extractBaseColorTextureBytes(allocator: std.mem.Allocator, gltf: *const zgltf.Gltf) ![]const u8 {
    if (gltf.data.materials.len == 0) {
        return error.GltfMissingMaterial;
    }
    const material = gltf.data.materials[0];
    const base_color = material.metallic_roughness.base_color_texture orelse
        return error.GltfMissingBaseColorTexture;

    const tex_index: usize = @intCast(base_color.index);
    if (tex_index >= gltf.data.textures.len) {
        return error.GltfTextureIndexOutOfRange;
    }
    const texture = gltf.data.textures[tex_index];
    const image_index = texture.source orelse return error.GltfTextureNoSource;
    if (image_index >= gltf.data.images.len) {
        return error.GltfImageIndexOutOfRange;
    }
    const image = gltf.data.images[image_index];
    const src = image.data orelse return error.GltfImageMissingData;
...
fn createPipeline(
    ...
    const attribute_descriptions = [_]vk.VertexInputAttributeDescription{
        ...
        .{ .binding = 0, .location = 2, .format = .r32g32_sfloat, .offset = 6 * @sizeOf(f32) },
    };
